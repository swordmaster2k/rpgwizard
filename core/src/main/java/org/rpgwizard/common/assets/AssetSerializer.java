...
public interface AssetSerializer {
    ...
    /**
     * Gets the priority of the serializer. Serializers sort in ascending order, so serializers with lower numbers take
     * priority over serializers with higher numbers. If two serializers have the same priority, sorting between them is
     * arbitrary. If you want to guarantee that one serializer comes before another, you must assign it a lower priority
     * number.
     *
     * @return the priority of the serializer; lower numbers come first (serializers are sorted in ascending order)
     */
    int priority();
