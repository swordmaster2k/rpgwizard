...
/**
 * A model that represents <code>Board</code> files in the RPGToolkit engine and editor. Used during the serialization
 * processes to various formats, at the moment it contains the old binary routines for opening 3.x formats which should
 * be removed in 4.1.
 *
 * @author Geoff Wilson
...
public final class Board extends AbstractAsset implements Selectable {
    ...
    /**
     * Geometric perspective of a board for use with directional movement, rendering, raycasting, and other mathematical
     * transformations.
     *
    ...
    public Board(Board board) {
        ...
        LinkedList<BoardLayer> layersCopy = new LinkedList<>();
        board.layers.forEach((layer) -> {
            layersCopy.add(new BoardLayer(layer, this));
        });
        layers = layersCopy;
    ...
    public void fireBoardLayerImageRemoved(BoardLayerImage image) {
        ...
        while (iterator.hasNext()) {
            ...
            ((BoardChangeListener) iterator.next()).boardLayerImageRemoved(event);
        }
    }
    ...
    /**
     * Invoked when the required TileSets exist in the Cache. When first loaded Boards will have blank Tile rasters.
     */
