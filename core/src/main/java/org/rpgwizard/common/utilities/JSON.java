...
//            out.println(json.toString());
	...
	/*
	 * public Person(File f) { //... this.harvestJSON(JSON.load(f)); //... }
	 * 
	 * @Override public boolean save() { File f = new File(filePath); return
	 * JSON.save(this, f); }
	 * 
	 ...
	 */
	/*
	 * public static Person open(String fileName) { File f = new
	 * File(GameBase.assetsDir + charactersDir + fileName + sep + fileName +
	 * ".chr"); return new Person(f); }
	 * 
	 * @Override public String toJSONString() { JSONStringer json = new
	 * JSONStringer(); json.object(); this.populateJSON(json); json.endObject();
	 * return JSON.toPrettyJSON(json); }
	 * 
	 * @Override public void populateJSON(JSONStringer json) {
	 * json.key("name").value(this.name); json.key("race").value(this.race);
	 * json.key("jump").value(this.jump); //...etc }
	 * 
	 ...
	 */
	/*
	 * private void harvestJSON(JSONObject json) { this.name =
	 * json.optString("name", "???"); this.race = json.getString("race");
	 * this.jump = json.optInt("jump", 1); //...etc } }
	 * 
	 * //////////////////////// // Example Subclass: // //////////////////////
	 * //public class Combatant extends Person { /** Create a new Combatant from
	 * a file and return it.
	 * 
	 * @param fileName the filename (in the format "filename") to load from; the
	 * rest of the path to that file is calculated automatically
	 * 
	 * @return a new Combatant
	 */
	/*
	 * public static Combatant open(String fileName) { File f = new
	 * File(GameBase.assetsDir + charactersDir + fileName + sep + fileName +
	 * ".chr"); return new Combatant(f); }
	 * 
	 * public Combatant(File f) { super(f); this.harvestJSON(JSON.load(f));
	 * //... }
	 * 
	 * @Override public String toJSONString() { JSONStringer json = new
	 * JSONStringer(); json.object(); super.populateJSON(json);
	 * this.populateJSON(json); json.endObject(); return
	 * JSON.toPrettyJSON(json); }
	 * 
	 * @Override public void populateJSON(JSONStringer json) {
	 * json.key("hp").value(this.maxHealth);
	 * json.key("charge").value(this.maxCharge); //...etc
	 * 
	 * json.key("abilities").array(); for(Ability a : this.abilityList) {
	 * json.value(a.name); } json.endArray(); //...etc } private void
	 * harvestJSON(JSONObject json) { this.maxHealth = json.optInt("hp", 100);
	 * this.maxCharge = json.optInt("charge", 100); //..etc
	 * 
	 * JSONArray array = json.optJSONArray("abilities"); if(array != null) {
	 * for(int i = 0; i < array.length(); i++) { this.abilityList.add(new
	 * Ability(array.getString(i))); } } //...etc } }
	 */
