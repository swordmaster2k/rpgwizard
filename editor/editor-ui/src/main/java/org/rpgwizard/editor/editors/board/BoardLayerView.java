...
public class BoardLayerView {
	 ...
	 *
	 * @param visible
	 *            <code>true</code> to make the layer visible;
	...
	public static BufferedImage getPlaceHolderSprite() {
		return Icons.toBufferedImage(Icons
				.getLargeIcon("icons8-add-user-female-48"));
	}
	 ...
	 *             Throws an exception if the tiles pixel value is out of the
	 *             allowed range.
	 */
		...
		for (int x = 0; x < parentBoard.getWidth(); x++) {
			for (int y = 0; y < parentBoard.getHeight(); y++) {
				if (layer.getTiles()[x][y] != null) {
					...
					g.drawImage(tile.getTileAsImage(), (x * layer.getBoard()
							.getTileWidth()), (y * layer.getBoard()
							.getTileHeight()), null);
				} else {
	...
	private void drawVectorLines(Graphics2D g, BoardVector vector) {
		...
		for (int i = 0; i < count - 1; i++) {
			int[] points = GuiHelper.ensureVectorVisible(layer.getBoard(),
					vector.getPointX(i), vector.getPointY(i),
		...
		if (vector.isClosed()) {
			...
			int[] points = GuiHelper.ensureVectorVisible(layer.getBoard(),
					vector.getPointX(count - 1), vector.getPointY(count - 1),
					vector.getPointX(0), vector.getPointY(0));
			g.drawLine(points[0], points[1], points[2], points[3]);
		}
	}
