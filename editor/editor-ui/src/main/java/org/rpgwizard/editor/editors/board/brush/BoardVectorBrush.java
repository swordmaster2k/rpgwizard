...
public class BoardVectorBrush extends AbstractBrush {
	...
	public void drawPreview(Graphics2D g2d, AbstractBoardView view) {
		...
		Point lastVectorPoint = boardVector.getPoints().get(
				boardVector.getPoints().size() - 1);
		...
		int[] points = GuiHelper.ensureVectorVisible(view.getBoard(),
				lastVectorPoint.x, lastVectorPoint.y, coordinates[0],
				coordinates[1]);
	...
	public void finish() {
		if (boardVector.getPointCount() < 2) {
			affectedContainer.getLayer(currentLayer).getLayer().getVectors()
					.remove(boardVector);
		}
	...
	public void doMouseButton2Pressed(Point point,
		...
		if (editor instanceof BoardEditor) {
			...
			RemoveVectorAction action = new RemoveVectorAction(boardEditor,
					point.x, point.y);
	...
	public void doMouseButton3Pressed(Point point,
		...
		if (editor instanceof BoardEditor) {
			...
			{
				selectVector(
						boardEditor.getBoardView().getCurrentSelectedLayer()
								.getLayer().findVectorAt(point.x, point.y),
						boardEditor);
			}
		}
