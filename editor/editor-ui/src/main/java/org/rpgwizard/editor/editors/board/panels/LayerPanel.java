...
public class LayerPanel extends JPanel
		...
		if (e.getSource().equals(opacitySlider)) {
			if (boardView != null) {
				if (layerTable.getSelectedRow() > -1
					...
					boardView
							.getLayer(
									(boardView.getBoard().getLayers().size() - layerTable
											.getSelectedRow()) - 1).setOpacity(
									opacitySlider.getValue() / 100.0f);
				}
			}
		}
	}
	...
	public void valueChanged(ListSelectionEvent e) {
		...
		if (boardView != null) {
			if (layerTable.getSelectedRow() > -1) {
				BoardLayerView selectedLayer = boardView.getLayer((boardView
						.getBoard().getLayers().size() - layerTable
						.getSelectedRow()) - 1);

				boardView.setCurrentSeletedLayer(selectedLayer);

				opacitySlider
						.setValue((int) (selectedLayer.getOpacity() * 100));
			}
		}
	}
	...
	private void init() {
		...
		sliderPanel.setMaximumSize(new Dimension(Integer.MAX_VALUE, sliderPanel
		...
		if (boardView != null) {
			...
			if (boardView.getBoard().getLayers().size() > 0) {
				layerTable.setRowSelectionInterval(layerTable.getModel()
						.getRowCount() - 1, 0);
			}
		} else {
			...
			public void actionPerformed(ActionEvent e) {
				if (boardView != null) {
					if (layerTable.getSelectedRow() > 0) {
						...
						boolean result = boardView
								.getBoard()
								.moveLayerUp(
										-(selectedIndex
												- layerTable.getRowCount() + 1));
						if (result) {
							layerTable.setRowSelectionInterval(
			...
			public void actionPerformed(ActionEvent e) {
				if (boardView != null) {
					if (layerTable.getSelectedRow() > -1) {
						...
						boardView.getBoard()
								.cloneLayer(
										(boardView.getBoard().getLayers()
												.size() - selectedIndex) - 1);
						layerTable.setRowSelectionInterval(selectedIndex,
								selectedIndex);
					}
				}
			}
