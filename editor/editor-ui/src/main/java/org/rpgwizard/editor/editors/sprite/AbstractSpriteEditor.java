...
public abstract class AbstractSpriteEditor extends AssetEditorWindow
	...
	protected static final List<String> STANDARD_PLACE_HOLDERS = Arrays.asList(
			"SOUTH", "NORTH", "EAST", "WEST", "NORTH_WEST", "NORTH_EAST",
			"SOUTH_WEST", "SOUTH_EAST", "ATTACK", "DEFEND", "SPECIAL_MOVE",
			"DIE", "REST");

	protected static final List<String> STANDING_PLACE_HOLDERS = Arrays.asList(
			"SOUTH_IDLE", "NORTH_IDLE", "EAST_IDLE", "WEST_IDLE",
			"NORTH_WEST_IDLE", "NORTH_EAST_IDLE", "SOUTH_WEST_IDLE",
			"SOUTH_EAST_IDLE");
	...
	public void setAnimationsTableModel(
			AnimationsTableModel animationsTableModel) {
	...
	public void updateAnimatedPanel() {
		...
		if (selectedAnim != null) {
			...
			animatedPanel.setActivationVectorOffset(sprite
					.getActivationVectorOffset());
		}

		try {
			animatedPanel.setAnimation(selectedAnim);
		} catch (IOException ex) {
			Logger.getLogger(AbstractSpriteEditor.class.getName()).log(
	...
	public void openAnimation(String path) {
		if (!path.isEmpty()) {
			File file = EditorFileManager.getPath(EditorFileManager
					.getTypeSubdirectory(Animation.class)
					+ File.separator
					+ path);
			if (file.exists()) {
	...
	protected void buildStatsPanel(List<Component> labels,
		...
		GroupLayout.ParallelGroup horizontalParallelGroup = statsLayout
				.createParallelGroup();
		...
		GroupLayout.SequentialGroup verticalSequentialGroup = statsLayout
		...
		if (!profileImagePath.isEmpty()) {
			profilePanel.addImage(new File(EditorFileManager.getGraphicsPath()
					+ profileImagePath));
		}
	...
	protected void checkProfileImagePath() {
		if (profilePanel.getFile() != null) {
			...
			String path = profilePanel.getFile().getAbsolutePath()
					.replace(remove, "");
