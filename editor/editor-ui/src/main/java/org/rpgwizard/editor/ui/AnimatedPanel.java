...
public class AnimatedPanel extends AbstractImagePanel
	...
	public void paint(Graphics g) {
		...
		if (animation != null) {
			...
			if (frameImage != null) {
				...
				if (frameImage.getWidth() > width
						|| frameImage.getHeight() > height) {
					makeSubImage();
				}
			...
			if (baseVector != null) {
				...
				drawVector(baseVector, g, x, y, (int) baseVectorOffset.getX(),
						(int) baseVectorOffset.getY());
			}

			if (activationVector != null) {
				g.setColor(Color.YELLOW);
				drawVector(activationVector, g, x, y,
						(int) activationVectorOffset.getX(),
	...
	private void drawVector(BoardVector vector, Graphics g, int x, int y,
			int xOffset, int yOffset) {
		int count = vector.getPointCount();
		for (int i = 0; i < count - 1; i++) {
			g.drawLine(x + vector.getPointX(i) + xOffset,
