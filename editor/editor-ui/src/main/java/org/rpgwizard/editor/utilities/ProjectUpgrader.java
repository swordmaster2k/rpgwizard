...
/**
 * Utility class for upgrading projects between versions, deals with any file
 * format changes that have been made.
 * 
...
public class ProjectUpgrader {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(ProjectUpgrader.class);
	...
	public static void upgrade(File path) {
		LOGGER.info("Upgrading project located at path=[{}]",
				path.getAbsolutePath());

		int filesUpgraded = 0;

		// Upgrade project file.
		String extension = CoreProperties
	...
	private static int upgradeFiles(String[] extensions, File path) {
		LOGGER.info("Upgrading files extensions=[{}], [path=[{}]", extensions,
				path);
		if (!path.exists()) {
			return 0;
		}
		...
		for (File file : files) {
			try {
				...
				if (checkVersion(asset)) {
					LOGGER.info("Upgrading asset file=[{}]",
							file.getAbsolutePath());
					FileTools.saveAsset(asset);
					counter++;
				}
			} catch (Exception ex) {
				LOGGER.error("Failed to upgrade file=[{}]",
	...
	private static boolean checkVersion(AbstractAsset asset) {
		...
		return asset.getVersion() == LEGACY_VERSION
				|| asset.getVersion() < CURRENT_VERSION;
	}
