    ...
    s(r[o]); return s; })({ 1: [function (require, module, exports) {
            ...
            Crafty.extend({
                ...
                multitouch: function (bool) {
                    if (typeof bool !== "boolean")
                        return this._touchHandler.multitouch;
                ...
                mouseDispatch: function (e) {
                    if (!Crafty.mouseObjs)
                        return;
                    Crafty.lastEvent = e;
                    var tar = e.target ? e.target : e.srcElement, closest, pos = Crafty.domHelper.translate(e.clientX, e.clientY), type = e.type;
                    ...
                    if (closest) {
                        ...
                        if (type === "mousedown") {
                            ...
                            closest.trigger(type, e); //trigger whatever it is
                    }
                    else {
                ...
                touchDispatch: function (e) {
                    if (!Crafty.touchObjs && !Crafty.mouseObjs)
                        return;
                ...
                _touchHandler: {
                    ...
                    handleMove: function (e) {
                        var touches = e.changedTouches;
                        for (var i = 0, l = touches.length; i < l; i++) {
                            var idx = this.fingerDownIndexById(touches[i].identifier), tar = e.target ? e.target : e.srcElement;
                    ...
                    handleEnd: function (e) {
                        var touches = e.changedTouches, eventName = e.type === "touchcancel" ? "TouchCancel" : "TouchEnd";
                        for (var i = 0, l = touches.length; i < l; i++) {
                ...
                findPointerEventTargetByComponent: function (comp, e, target) {
                    var tar = target ? target : Crafty.stage.elem, closest, current, q, l, i, pos, layerPos, maxz = -Infinity;
                    var x = e.clientX;
                    var y = e.clientY;
            ...
            Crafty.c("MouseDrag", {
                ...
                _ondrag: function (e) {
                    ...
                    if (!this._dragging || e.realX === 0 || e.realY === 0)
                        return false;
            ...
            Crafty.fn = Crafty.prototype = {
                init: function (selector) {
                    ...
                    if (typeof selector === "string") {
                        ...
                        e, //entity forEach
                        current, and = false, //flags for multiple
                        or = false, del, comps, score, i, l;
                        if (selector === '*') {
                            i = 0;
                            for (e in entities) {
            ...
            Crafty.extend = Crafty.fn.extend = function (obj) {
                ...
                for (key in obj) {
                    if (target === obj[key])
                        continue; //handle circular reference
                    target[key] = obj[key];
                }
                return target;
            };
                ...
                timer: (function () {
                    ...
                    return {
                        init: function () {
                            ...
                            var onFrame = (typeof window !== "undefined") && (window.requestAnimationFrame ||
                        ...
                        stop: function () {
                            ...
                            if (typeof tick !== "function")
                                clearInterval(tick);
                            var onFrame = (typeof window !== "undefined") && (window.cancelAnimationFrame ||
            ...
            (function (process) {
                ...
                (function testSupport() {
                    var support = Crafty.support = {}, ua = (typeof navigator !== "undefined" && navigator.userAgent.toLowerCase()) || (typeof process !== "undefined" && process.version), match = /(webkit)[ \/]([\w.]+)/.exec(ua) ||
                        /(o)pera(?:.*version)?[ \/]([\w.]+)/.exec(ua) ||
                        /(ms)ie ([\w.]+)/.exec(ua) ||
                        /(moz)illa(?:.*? rv:([\w.]+))?/.exec(ua) ||
                        /(v)\d+\.(\d+)/.exec(ua) || [], mobile = /iPad|iPod|iPhone|Android|webOS|IEMobile/i.exec(ua);
                    /**@
                     * #Crafty.mobile
                     * @comp Crafty.device
                     * @kind Property
            ...
            module.exports = {
                ...
                load: function (data, oncomplete, onprogress, onerror) {
                    ...
                    var j = 0, total = (data.audio ? Object.keys(data.audio).length : 0) +
                        (data.images ? Object.keys(data.images).length : 0) +
                        (data.sprites ? Object.keys(data.sprites).length : 0), current, fileUrl, obj, type, asset, paths = Crafty.paths(), getExt = function (f) {
                ...
                removeAssets: function (data) {
                    data = (typeof data === "string" ? JSON.parse(data) : data);
                    var current, fileUrl, type, asset, paths = Crafty.paths(), getFilePath = function (type, f) {
                        return (f.search("://") === -1 ? (type === "audio" ? paths.audio + f : paths.images + f) : f);
                    };
                    for (type in data) {
                        for (asset in data[type]) {
            ...
            Crafty._registerLayerTemplate("Canvas", {
                ...
                _render: function () {
                    var dirtyViewport = this._dirtyViewport, l = this._changedObjs.length, ctx = this.context;
                    if (!l && !dirtyViewport) {
                        return;
                    }
                ...
                _drawDirty: function (view) {
                    ...
                    var i, j, q, rect, len, obj, changed = this._changedObjs, l = changed.length, dirty = this._dirtyRects, rectManager = Crafty.rectManager, overlap = rectManager.overlap, ctx = this.context, dupes = [], objs = [];
                ...
                _drawAll: function (rect) {
                    ...
                    var q = Crafty.map.search(rect), i = 0, l = q.length, ctx = this.context, current;
                    ctx.clearRect(rect._x, rect._y, rect._w, rect._h);
                ...
                _clean: function () {
                    var rect, obj, i, l, changed = this._changedObjs;
                ...
                _createDirty: function (obj) {
                    var rect = obj._mbr || obj, dirty = this._dirtyRects, rectManager = Crafty.rectManager;
                    if (obj.staleRect) {
            ...
            Crafty.extend({
                ...
                domHelper: {
                    ...
                    innerPosition: function (obj) {
                        var rect = obj.getBoundingClientRect(), x = rect.left + (window.pageXOffset ? window.pageXOffset : document.body.scrollLeft), y = rect.top + (window.pageYOffset ? window.pageYOffset : document.body.scrollTop), 
                        //border left
                        borderX = parseInt(this.getStyle(obj, 'border-left-width') || 0, 10) || parseInt(this.getStyle(obj, 'borderLeftWidth') || 0, 10) || 0, borderY = parseInt(this.getStyle(obj, 'border-top-width') || 0, 10) || parseInt(this.getStyle(obj, 'borderTopWidth') || 0, 10) || 0;
                        x += borderX;
                        y += borderY;
                        return {
                            x: x,
            ...
            Crafty._registerLayerTemplate("DOM", {
                ...
                _setViewport: function () {
                    var style = this._div.style, view = this._viewportRect();
                    var scale = view._scale;
                    var dx = -view._x * scale;
                    var dy = -view._y * scale;
            ...
            Crafty.c("Sprite", {
                ...
                _setupSpriteImage: function (layer) {
                    if (!this.__image || !this.img || !layer)
                        return;
                    if (layer.type === "WebGL") {
                ...
                _drawSprite: function (e) {
                    ...
                    if (e.type === "canvas") {
                        ...
                        var vscale = this._h / co.h, hscale = this._w / co.w, style = this._element.style, bgColor = style.backgroundColor;
                        if (bgColor === "initial")
                            bgColor = "";
            ...
            Crafty.extend({
                ...
                viewport: {
                    ...
                    centerOn: function (targ, time) {
                        var x = targ.x + Crafty.viewport.x, y = targ.y + Crafty.viewport.y, mid_x = targ.w / 2, mid_y = targ.h / 2, cent_x = Crafty.viewport.width / 2 / Crafty.viewport._scale, cent_y = Crafty.viewport.height / 2 / Crafty.viewport._scale, new_x = x + mid_x - cent_x, new_y = y + mid_y - cent_y;
                     ...
                     */
                    reload: function () {
                        var w = window.innerWidth, h = window.innerHeight, offset;
            ...
            Crafty._registerLayerTemplate("WebGL", {
                ...
                render: function (rect) {
                    ...
                    var q = Crafty.map.search(rect), i = 0, l = q.length, current;
            ...
            Crafty.c("WebGL", {
                ...
                draw: function (ctx, x, y, w, h) {
                    ...
                    prog.writeVector("aPosition", this._x, this._y, this._x, this._y + this._h, this._x + this._w, this._y, this._x + this._w, this._y + this._h);
                    // Write orientation
                    prog.writeVector("aOrientation", this._origin.x + this._x, this._origin.y + this._y, this._rotation * Math.PI / 180);
                    // Write z, alpha
            ...
            Crafty.c("2D", {
                ...
                _rotate: function (v) {
                    ...
                    // ct = Math.cos(rad),
                    // st = Math.sin(rad),
                    cos = Math.cos(drad), sin = Math.sin(drad);
                    this.trigger("Rotate", {
            ...
            Crafty.polygon.prototype = {
                ...
                intersectRay: function (origin, direction) {
                    ...
                    var originX = origin._x, directionX = direction.x, originY = origin._y, directionY = direction.y;
                    var i = 0, l = points.length;
                    var startX = points[l - 2], endX, edgeX, startY = points[l - 1], endY, edgeY;
                    for (; i < l; i += 2) {
                        endX = points[i];
                        endY = points[i + 1];
                        edgeX = endX - startX;
            ...
            var Crafty = require('../core/core.js'), DEG_TO_RAD = Math.PI / 180, EPSILON = 1e-6;
            Crafty.extend({
                /**@
                 * #Crafty.raycast
                 * @category 2D
                ...
                raycast: function (origin, direction) {
                    // default parameters
                    var comp = 'obj', maxDistance = Infinity, sort = true;
                    // optional arguments
                    var argument, type;
                    for (var i = 2, l = arguments.length; i < l; ++i) {
                        ...
                        if (type === 'number')
                            maxDistance = argument + EPSILON; // make it inclusive
                        else if (type === 'string')
                            comp = argument;
                        else if (type === 'boolean')
                            sort = argument;
                    }
                    var ox = origin._x, oy = origin._y, dx = direction.x, dy = direction.y;
                    var alreadyChecked = {}, results = [];
                    if (maxDistance < 0) { // find first intersection
                        var closestObj = null, minDistance = Infinity;
                        // traverse map
            ...
            Crafty.c("Collision", {
                ...
                hit: function (component) {
                    var area = this._cbr || this._mbr || this, results = Crafty.map.search(area, false), i = 0, l = results.length, dupes = {}, id, obj, oarea, key, overlap = Crafty.rectManager.overlap, hasMap = ('map' in this && 'containsPoint' in this.map), finalresult = [];
                    if (!l) {
                        return null;
                    }
                    for (; i < l; ++i) {
                ...
                _SAT: function (poly1, poly2) {
                    var i = 0, points1 = poly1.points, points2 = poly2.points, l = points1.length / 2, j, k = points2.length / 2, nx = 0, ny = 0, length, min1, min2, max1, max2, interval, MTV = -Infinity, MNx = null, MNy = null, dot, np;
            ...
            var __motionVector = function (self, prefix, setter, vector) {
                var publicX = prefix + "x", publicY = prefix + "y", privateX = "_" + publicX, privateY = "_" + publicY;
                if (setter) {
            ...
            Crafty.c("AngularMotion", {
                ...
                _angularMotionTick: function (frameData) {
                    ...
                    var oldR = this._rotation, vr = this._vrotation, ar = this._arotation;
                    // s += v * Δt + (0.5 * a) * Δt * Δt
            ...
            Crafty.c("Motion", {
                ...
                ccdbr: function (ccdbr) {
                    var pos = this._cbr || this._mbr || this, dx = this._dx, dy = this._dy, ccdX = 0, ccdY = 0, ccdW = dx > 0 ? (ccdX = dx) : -dx, ccdH = dy > 0 ? (ccdY = dy) : -dy;
                    ccdbr = ccdbr || {};
                ...
                _linearMotionTick: function (frameData) {
                    var dt = frameData.dt / 1000; // time in s
                    var oldX = this._x, vx = this._vx, ax = this._ax, oldY = this._y, vy = this._vy, ay = this._ay;
                    // s += v * Δt + (0.5 * a) * Δt * Δt
                    ...
                    this.vy = vy + ay * dt;
                    // Check if direction of velocity has changed
                    var oldDirection = this.__oldDirection, _vx = this._vx, dvx = _vx ? (_vx < 0 ? -1 : 1) : 0, // A quick implementation of Math.sign
            ...
            Crafty.c("Supportable", {
                ...
                _detectGroundTick: function () {
                    var groundComp = this._groundComp, ground = this._ground, overlap = Crafty.rectManager.overlap, area;
                    ...
                    if (!ground) {
                        var obj, oarea, results = Crafty.map.search(area, false), i = 0, l = results.length;
                        for (; i < l; ++i) {
                            obj = results[i];
                ...
                rectManager: {
                    ...
                    */
                    ...
                    boundingRect: function (set) {
                        if (!set || !set.length)
                            return;
                        var i = 1, l = set.length, current, master = set[0], tmp;
            ...
            HashMap.prototype = {
                ...
                search: function (rect, filter) {
                    ...
                    if (filter === undefined)
                        filter = true; //default filter to true
                ...
                traverseRay: function (origin, direction, callback) {
                    ...
                    var minCol = keyBounds.min.x, minRow = keyBounds.min.y, maxCol = keyBounds.max.x, maxRow = keyBounds.max.y;
                    // direction to traverse cells
                    var stepCol = dirX > 0 ? 1 : (dirX < 0 ? -1 : 0), stepRow = dirY > 0 ? 1 : (dirY < 0 ? -1 : 0);
                    // first, next cell edge in absolute coordinates
                    var firstCellEdgeX = (dirX >= 0) ? (currentCol + 1) * cellsize : currentCol * cellsize, firstCellEdgeY = (dirY >= 0) ? (currentRow + 1) * cellsize : currentRow * cellsize;
