...
public final class EmbeddedBrowser extends JFrame {
	...
	public EmbeddedBrowser(String title, String startURL, boolean useOSR,
		...
		cefClient.addLoadHandler(new CefLoadHandler() {
			@Override
			public void onLoadingStateChange(CefBrowser arg0, boolean arg1,
						...
						if (event.windows_key_code == KeyEvent.VK_F12) {
							if (event.type == CefKeyboardHandler.CefKeyEvent.EventType.KEYEVENT_RAWKEYDOWN) {
								if (devToolsUI == null) {
									...
									devToolsUI.setVisible(!devToolsUI
											.isVisible());
								}
								validate();
			...
			@Override
			public void onLoadError(CefBrowser arg0, int arg1,
					CefLoadHandler.ErrorCode arg2, String arg3, String arg4) {
			}
		});
