 ...
 */
@Mapper
public abstract class OldEnemyToSpriteMapper extends AbstractAssetMapper {
    
    @BeforeMapping
    protected void handleColliderEnabled(OldEnemy source, @MappingTarget Sprite target) {
        target.getCollider().setEnabled(!source.isBaseVectorDisabled());
    }
    
    @BeforeMapping
    protected void handleTriggerEnabled(OldEnemy source, @MappingTarget Sprite target) {
    ...
    public abstract Sprite map(OldEnemy source);
