 ...
 */
@Mapper
public abstract class OldNpcToSpriteMapper extends AbstractAssetMapper {
    
    @BeforeMapping
    protected void handleColliderEnabled(OldNpc source, @MappingTarget Sprite target) {
        target.getCollider().setEnabled(!source.isBaseVectorDisabled());
    }
    
    @BeforeMapping
    protected void handleTriggerEnabled(OldNpc source, @MappingTarget Sprite target) {
    ...
    public abstract Sprite map(OldNpc source);
